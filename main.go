		...
		if logFile[len(logFile)-4:] == ".csv" {
			...
			for _, record := range records {
				...
				l := Log{
					...
					Service: record[2],
					Message: record[3],
					Sources: []Source{}, // Add sources if available
				}
				...
				// Search for message source files in the current directory and subdirectories
				if m != "" {
					cmd := exec.Command("rg", "-F", m, "--glob", "!**/*.csv")
					...
					if err != nil {
						if strings.Contains(err.Error(), "exit status 1") {
							...
							l.Sources = append(l.Sources, Source{
								Path: "No sources found for this log...",
								Line: 0,
							})
						} else {
				...
				p.Send(logProcessingMsg{
					Progress: len(logs) * 100 / len(records),
					Logs:     logs,
					LogTable: table.New(),
				})
